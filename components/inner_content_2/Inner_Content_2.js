...
const Inner_content_2 = () => {
  const [click, setClick] = useState(true);
  const [isShadow, setIsShadow] = useState(false);
  const percentage = 66;
  const store = useSelector((state) => state.reducer.Innner_context);
  return (
    <div className={`${style.Inner_content} ${style.Inner_Content_2}`}>
      <div id={style.Inner_Content_2} className="container">
...
export default Inner_content_2;
